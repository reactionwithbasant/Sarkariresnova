...
async function start() {
    ...
    try {
        console.log("🚀 RAGHVITA ENGINE: Launching AdSense Pro Mode...");
        ...
        const prompt = `Write a professional 800-word Hindi Article for ONE specific ${randomCatFinal} 2026. 
        STRICT FORMAT:
        [TITLE] Clear Post Name
        [SLUG] unique-slug-2026
        [CONTENT] 
        - Detailed Intro.
        - Table for Dates & Fees.
        - Table for Eligibility.
        - Detailed Step-by-Step 'How to Apply' (Avedan Kaise Karein) in points.
        - 5 Standard FAQs for AdSense.
        [APPLY_LINK] Direct official registration portal URL (not google.com).
        [OFFICIAL_LINK] Main department website URL.`;
        ...
        const officialLink = (rawText.match(/\[OFFICIAL_LINK\]\s*(https?:\/\/[^\s*]+)/i) || [])[1] || "https://www.google.com";
        ...
        <style> table { width: 100%; border-collapse: collapse; margin: 20px 0; } th, td { border: 1px solid #1e3a8a; padding: 12px; text-align:center; } th { background: #1e3a8a; color: white; } </style>
            ...
            <main class="max-w-4xl mx-auto bg-white p-6 mt-6 shadow-2xl border-t-8 border-red-600 rounded-b-lg">
                ...
                <img src="${imageUrl}" alt="Official Notification" class="w-full h-auto rounded-lg mb-8 shadow-md">
                ...
                <div class="mt-12 p-8 bg-blue-50 border-2 border-blue-100 rounded-2xl text-center">
                    <h3 class="font-bold text-xl text-blue-900 mb-4 italic underline">Important Official Links</h3>
                    <a href="${applyLink}" target="_blank" rel="noopener noreferrer" class="block w-full bg-red-600 text-white p-5 rounded-xl font-bold text-2xl shadow-xl hover:bg-red-700 transition-all mb-4">👉 CLICK HERE TO APPLY ONLINE</a>
                    <a href="${officialLink}" target="_blank" rel="noopener noreferrer" class="block w-full bg-gray-700 text-white p-3 rounded-lg font-bold text-lg shadow-md hover:bg-gray-800">🌐 VISIT OFFICIAL WEBSITE</a>
                </div>
            </main>
        ...
        console.log("✅ Success! Termux restarted and code is live.");
