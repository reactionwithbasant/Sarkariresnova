...
async function start() {
    ...
    try {
        console.log("🚀 RAGHVITA ENGINE: Fixing Content & Image Display...");
        ...
        const prompt = `Act as a Sarkari Exam SEO Expert. Write a 600-word Hindi update for ONE SPECIFIC ${randomCatFinal} 2026. 
        Structure:
        [TITLE] Title of the Post
        [SLUG] short-slug-2026
        [CONTENT] Write full details in HTML: Intro, Table for Dates/Fees, Table for Eligibility, How to Apply steps, and FAQ.
        [APPLY_LINK] Direct Registration URL.`;
        ...
        // --- IMAGE FIX ---
        const imageUrl = "https://images.unsplash.com/photo-1622243503145-56fd1d507424?auto=format&fit=crop&q=80&w=800";
        ...
        <style> table { width: 100%; border-collapse: collapse; margin: 20px 0; } th, td { border: 1px solid #1e3a8a; padding: 10px; text-align:center; } th { background: #1e3a8a; color: white; } </style>
            ...
            <main class="max-w-4xl mx-auto bg-white p-6 mt-4 shadow-2xl border-t-8 border-red-600">
                <div class="mb-4 flex justify-between"><span class="bg-red-600 text-white px-3 py-1 rounded font-bold text-xs uppercase">${randomCatFinal}</span></div>
                <h2 class="text-3xl font-extrabold text-blue-900 mb-6">${title}</h2>
                <img src="${imageUrl}" alt="Sarkari Update" class="w-full h-auto rounded-lg mb-8 shadow-md">
                ...
                <div class="mt-10 p-6 bg-yellow-50 border-2 border-yellow-200 rounded-xl text-center">
                    <a href="${applyLink}" target="_blank" rel="noopener noreferrer" class="block w-full bg-red-600 text-white p-5 rounded-lg font-bold text-2xl shadow-xl hover:bg-red-700">👉 CLICK HERE TO APPLY ONLINE</a>
                </div>
            </main>
        ...
        console.log("✅ Fixed! Content and Image are now displaying correctly.");
